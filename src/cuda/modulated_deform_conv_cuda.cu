...
__global__ void createBatchGemmBufferModulated(const float **input_b, float **output_b,
                    ...
                    const int deformable_group)
{
    ...
    createBatchGemmBufferModulated<<<grid, block, 0, THCState_getCurrentStream(state)>>>(
        input_b, output_b,
        columns_b, ones_b,
        weight_b, bias_b,
        input.data<scalar_t>(),
...
__global__ void createBatchGemmBufferModulatedBackward(
    float **grad_output_b,
    float **columns_b,
    float **ones_b,
    float **weight_b,
